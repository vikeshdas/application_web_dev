"""
Views to handle user releted operation like create a new user
,update information of a user,delete a user and list users of
a client
"""
...
class CustomPagination(PageNumberPagination):
    ...
    Attributes:
        page_size (int): The default number of items to include in
        each page. page_size_query_param (str): The name of the query
        parameter that allows clients to set the page size.
        max_page_size (int): The maximum number of items allowed
        in each page
    """
...
class UserView(APIView):
    ...
    def patch(self, request: HttpRequest) -> JsonResponse:
        ...
        Returns(JsonResponse): returns a message in Json format
        either successfully updated or error.
        """
        try:
            ...
            if "Duplicate entry" in str(e) and "for key 'client_user.username'" in str(
                e
            ):
                return JsonResponse(
                    {"error": "This username already exists"}, status=409
                )
    ...
    def put(self, request: HttpRequest) -> JsonResponse:
        """
        Creating a new user with unique username and contact
        number,means saving information of a user in database.

        Args:
            request:HttpRequest's object contains information of a user to save in database.

        Returns:
            JsonResponse : Returns message in JSON format either
            data saved successfully or failed.
        """
        data = json.loads(request.body)
        try:
            try:
...
class Users(APIView):
    """
    A view class handles operations related to more than one
    user.like get information of all users related to a client.
    ...
    def get(self, request: HttpRequest) -> JsonResponse:
        """
        Fetch all users of a client based on client_id .
        if client exist in database

        Args:
            request: HttpRequest's object contains client_id whose
            all user need to fetch from database.

        Return: information of all user of a client in the JSON
        format or error if client is invalid.
        ...
        if not Client.objects.filter(id=client_id):
            return JsonResponse(
                {"error": "There is no such client"}, status=409, safe=False
            )
        try:
